...
export default function Participants_Article({league}) {
  ...
  for(let i=0; i<league.teams.length; i++) {
    lis.push(<li style={{float:'left', marginRight:'30px', fontFamily:'RIXGOL', color:'white'}}><Link style={{color:'white', textDecoration:'none'}} to={'/championship/'+ league.teams[i].name }>{league.teams[i].name}</Link></li>)
  }
  return (
    <StyledDiv>
