...
&:before {
  ...
  border-top:1px rgba(255,255a,255,0.3) solid;
...
export default function Group_Article({teams}) {
    const src=process.env.PUBLIC_URL + '/images/teams/'+teams.name[0]+'.jpg';
  return (
    <StyledLink to={'/championship/'+ teams.name[0] }>
        <p style={{position:'absolute', height:'50%',left:'1%' ,fontSize:'50px', fontWeight:'', color:'white'}}>{teams.group[1]}</p>
        ...
        <div style={{position:'absolute', left:'35%'}}>
            ...
            <p style={{margin:'0', color:'rgba(255,255,255,0.6)', fontFamily:'ColfaxB'}}>{teams.group[2]}</p>
        </div>
    </StyledLink>
  )
}
